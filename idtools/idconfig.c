...
#include "defines.h"
/*
 * NB: the historical idconfig included "sys/conf.h" and <sys/mkdev.h> here.
 * It never actually references anything from them at its own compile time --
 * the device-switch structures and makedev() macros it cares about are emitted
 * as text into the *generated* conf.c (which the kernel build compiles against
 * the real kernel headers).  Pulling the kernel headers in here only clashes
 * with the host libc (e.g. pid_t), so they are intentionally dropped.
 *
 * The handful of values idconfig genuinely needs from <sys/mkdev.h> (the
 * legacy SVR3 minor type and its bounds, used only to clamp the default
 * MAXMINOR tunable) are reproduced here verbatim so we stay self-contained.
 */
#ifndef OMAXMIN
...
/*
 * 'errno' is declared by <errno.h> (included up top).  The historical
 * 'extern int errno;' is wrong on any modern libc, where errno is a
 * thread-local reached through a macro -- the bare extern makes the compiler
 * emit a direct data relocation against a symbol that no longer is one, which
 * the link-editor rejects (R_X86_64_PC32 against 'errno').
 */
...
rdmdevice()
{
        ...
        register FILE *fp;
        register struct t2 *q;
        struct multmaj mm;	/* not 'register': we take its address below */
	int start, end;
...
#endif /* vax || uts */
	int ntune;
	/* char *malloc(); -- provided by <stdlib.h> */
...
char *dev, *maj;
{
	...
	if (INSTRING(p->master->type, NEWDRV)) {
		if ((maxminor = pfind("MAXMINOR")) == (struct t3 *)NULL) {
			sprintf(errbuf, NOMAXMINOR, (unsigned long)highminor);
			warning(0);
		} else 
...
prfiles()
{
        ...
        for (p = devinfo, i = 0; i <= dbound; i++, p++)
        {
                ...
                if( p->conf == 0 )      /* not configured 'N'  */
                        wr_file(0, p->dev, pp, p->type);
                else                    /* configured 'Y' */
                        wr_file(1, p->dev, pp, p->type);
        }
        for( f=fstype, i=0; i<=fbound; i++, f++ )
        {
                /* filesystem types always supply a Driver.o (no NODRIVE flag) */
                if( f->fs_conf[0] == 'Y' ) /* no stubs.c files here */
                {
                        if( strcmp(f->fs_prefix,"du") == 0 )
                                wr_file(1, "dufst", pp, (char *)NULL);
                        else
                                wr_file(1, f->fs_name, pp, (char *)NULL);
                } else
			wr_file(0, f->fs_name, pp, (char *)NULL);
        }
        fclose(pp);
 ...
 */
wr_file(flag,dirname,ifp,type)
int flag;
char *dirname;
FILE *ifp;
char *type;	/* mdevice type field, or NULL when not applicable (fstypes) */
{
        struct stat bstat;
        char buff[100];
        int nodriver;
        ...
        /*
         * A configured module may legitimately have no Driver.o: it exists only
         * to contribute a space.c (e.g. gendisp, which just initialises the
         * dispatcher's slice_size tunable).  The 'N' (NODRIVE) flag in the
         * mdevice type field declares that explicitly, so we don't demand a
         * Driver.o and don't silently accept a missing one for real drivers.
         */
        nodriver = (type != NULL && INSTRING(type, NODRIVE));
        ...
        else if (!nodriver) {   /* Require a Driver.o if 'Y' in sdevice  */
