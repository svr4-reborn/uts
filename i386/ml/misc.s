...
/* (as it will appear in the address mapping of the new process). */
	movl	u_procp, %eax		# Is the old process same as
...
/* copy reference and modify bits from usertable[] to p_ubptbl[] */
	pushl	%esi
	pushl	%edi
	movl	u_tss, %esi		# The ljmp below will save the current
...
bzeroba:
	...
	andl	$3, %ecx		# do mod 4
	repz
	stosb				# %ecx contains residual bytes to clear
bzdone:
	popl	%edi
	popl	%ebp
	ret
...
/* Go to 1) */
/* 3) Count of bytes is less than WORD_BYTES */
/* copy bytes */
...
allcopy:
	...
	cmpl	$WORD_BYTES, %ebx		# are there less bytes to copy
/* than in a word ? */
	jl	bcptail			# treat it like the tail at end
/* of a non-aligned bcopy */
	testl	$WORD_BYTES-1, %esi		# is source address word aligned?
...
/* */
	...
	andl	$WORD_BYTES-1, %ecx		# get count of bytes to copy to get %esi
/* word aligned */
	subl	%ecx, %ebx		# decrement count of bytes to copy
	repz
	movsb				# move bytes from %ds:si -> %es:di
...
bcpalign:
	...
	andl	$WORD_BYTES-1, %ebx		# %ebx gets remainder bytes to copy
	repz
	movsl				# copy words
