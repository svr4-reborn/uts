...
int fpukill(int *regptr) {
  ...
  if ((regptr[CS] & 0xffff) != FPESEL) {
    cmn_err(CE_WARN, "Coprocessor error not from emulator, CS=0x%x\n",
            regptr[CS] & 0xffff);
    return (0);
  }
...
void fpeinit(void) {
    ...
    for (i = 0; i < (int)ehdrp->e_phnum; i++) {
      ...
      case PT_LOAD: /* Should be ELF fp text */
        ...
        if ((phdr->p_flags & PF_X) && !(phdr->p_flags & PF_W)) {
          ...
          em_data.ux_entloc = (caddr_t)ehdrp->e_entry;
  ...
  bad3:
     ...
     */
    if (getcoffhead(exhda.vp, &em_data, &execsz, &exhda) != 0) {
...
read_fp_text:
  ...
  error = vn_rdwr(UIO_READ, exhda.vp, fpeloc, em_data.ux_tsize,
                  (off_t)em_data.ux_toffset, UIO_SYSSPACE, 0, (ulong)0,
                  (struct cred *)NULL, &resid);
...
int fpeclean(void) {
  ...
  if (u.u_ar0[SS] != USER_DS) {
    ...
    dp = (char *)(((struct dscr *)(u.u_procp->p_ldt)) + seltoi(u.u_ar0[SS]));
    base &= 0xFFFF;
